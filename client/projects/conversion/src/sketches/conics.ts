...
export namespace Conics {
  ...
  export function initSketch(p: p5): void {
    ...
    function selectObj(e: MouseEvent) {
    ...
    function test(m: p5.Vector, v: p5.Vector, t: number): boolean {
      return t ** 2 > (v.x - m.x) ** 2 + (v.y - m.y) ** 2;
    }

    function toVectorSpace(m: p5.Vector): void {
    ...
    function releaseObj(e: MouseEvent): void {
      console.log(e);
      selected = null;
    }
    ...
    function intersect(v0: p5.Vector, v: p5.Vector,
                       u0: p5.Vector, u: p5.Vector): p5.Vector {
      let t = ((v0.x * u.y - u0.x * u.y) - (v0.y * u.x - u0.y * u.x)) /
              (v.y *  u.x - v.x * u.y);
      let o = v0.copy();
      let d = v.copy();
      if (t && isFinite(t)) {
